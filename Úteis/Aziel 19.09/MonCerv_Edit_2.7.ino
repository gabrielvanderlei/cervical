...
void loop()
{
  ...
  compX = cos(compPitch/degconvert)*sin(compYaw/degconvert);
  compY = cos(compPitch/degconvert)*cos(compYaw/degconvert);
  compZ = cos(compYaw/degconvert)*sin(compPitch/degconvert);

  String dataString = "Informacoes em relacao ao corpo Roll:" + String(angulodifer_Roll) + ", Pitch:" + String(angulodifer_Pitch) + ", Yaw:" + String(angulodifer_Yaw) + ", " + String(tempo);
  String nada = "////";
  Serial.print(compPitch);
  Serial.print(nada);
  Serial.println(compY);
  Bluetooth.println(1);
  //Essa linha Î¹ responsÎ±vel por ligar o led/buzzer quando a diferenÎ·a de Î²ngulos superar o angulo dado como crÃ­tico atravez dfo aplicativo.
  if (angulodifer_Roll >= dife | angulodifer_Pitch >= 60){
